...
describe('Regression Tests', () => {
  ...
  describe('REG-001: Pairing Code Validation Bypass', () => {
    it('should FAIL with current code — any code accepted', () => {
      // This test documents the current bug at line 291-292 of ConnectionManager.kt
      // Current code: val success = true  (ALWAYS ACCEPTS)

      const displayedCode = '1234';
      const receivedCode = '9999'; // WRONG CODE

      // Current broken logic
      const success = true; // BUG: always true

      // This test should FAIL — we want it to reject wrong code
      expect(success).toBe(false); // THIS FAILS with current code
    });
