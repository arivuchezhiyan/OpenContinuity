...
export interface ClipboardSyncPayload {
  ...
  /** Source device ID — receivers skip messages with their own deviceId to prevent echo */
  deviceId?: string;
  /** SHA-256 hex of content for deduplication */
  contentHash?: string;
}
...
// ============== File Chunk Messages (AirDrop-style chunked transfer) ==============

export interface FileChunkPayload {
  /** Same transferId used in FILE_TRANSFER_REQUEST */
  transferId: string;
  /** 0-based chunk index */
  sequence: number;
  /** Total number of chunks */
  totalChunks: number;
  /** Base-64 encoded raw bytes of this chunk */
  data: string;
  /** SHA-256 hex of this chunk's raw bytes for per-chunk integrity */
  checksum: string;
}
...
export interface DragFileStartPayload {
  /** Unique drag session identifier */
  dragId: string;
  /** Device originating the drag */
...
/** Bump minor for backward-compatible additions; major for breaking changes */
...
// Re-export protocolJson helper for symmetry
